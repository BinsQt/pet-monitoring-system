<section>
    <div class="fixed w-11/12 h-16 max-w-lg -translate-x-1/2 rounded-full bottom-4 left-1/2 bg-blue-800 z-50">
        <div class="flex justify-evenly h-full max-w-lg grid-cols-5 mx-auto w-full">
            <a href="/dashboard" class="w-full inline-flex flex-col items-center justify-center hover:bg-blue-50 group {{ request()->is('dashboard') ? 'bg-blue-50':''}} {{ request()->is('dashboard_') ? 'rounded-l-full':''}} {{ request()->is('dashboard_') ? 'bg-blue-50':''}} {{ request()->is('dashboard_') ? 'rounded-l-full':''}}">
